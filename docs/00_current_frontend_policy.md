...
`/player-analysis-clips`의 역할은 다음과 같다.
...
`/player-analysis-clips/new`의 역할은 다음과 같다.
...
`/player-analysis-clips/:playerClipId/edit`의 역할은 다음과 같다.
...
`viewCount`는 전체 재생 횟수나 지도자/분석관 조회 횟수가 아니다.

`viewCount`는 해당 클립의 대상 선수가 본인 개인 분석 클립 상세를 조회한 횟수다.

따라서 화면 문구는 단순 `조회 횟수`가 아니라 다음처럼 표시한다.

```text
대상 선수 조회 횟수
```

조회 기록이 없을 때는 다음 기준으로 안내한다.

```text
아직 {선수명} 선수가 이 개인 분석 클립을 조회하지 않았습니다.
```

조회 기록 API 실패 시 조회 기록 영역에 사용자 친화적 에러 메시지를 표시한다.

관리용 조회 기록 API는 다음을 사용한다.

```http
GET /api/management/players/{playerId}/player-analysis-clip-views?page=0&size=50
```

선수 본인 조회 기록 API는 백엔드에 존재하지만 현재 프론트 화면에는 노출하지 않는다.

```http
GET /api/player/me/player-analysis-clip-views?page=0&size=20
```

조회 기록 관련 프론트 파일은 다음과 같다.
...
다음 프론트 작업은 선수 기록 관리 프론트 연동이다.

다음 작업의 핵심 방향은 다음과 같다.

* 선수 기록 관리 백엔드 API를 프론트에 연결한다.
* 지도자/분석관이 선수별 기록을 관리할 수 있게 한다.
* 선수는 본인 기록만 조회할 수 있게 한다.
* PC 관리 화면과 모바일 선수 화면 사용성을 구분한다.
* 백엔드 DTO 필드와 API 주소를 실제 코드 기준으로 확인한 뒤 프론트 타입/API를 작성한다.
* 권한 검증은 백엔드에서 처리하고, 프론트는 버튼과 영역 노출만 제어한다.

다음 작업에서 우선 확인할 파일은 다음과 같다.
