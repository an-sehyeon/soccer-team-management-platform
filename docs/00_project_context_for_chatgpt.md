...
다음 작업으로 선수 기록 관리 프론트 연동을 진행하는 것이 적절하다.
...
```text
선수 개인 분석 클립 조회 기록 프론트 연동
```

완료된 핵심 정책은 다음과 같다.

* 선수 개인 분석 클립 조회 기록 백엔드 API를 프론트에 연결했다.
* GitHub `main` 기준 조회 기록 Service, Validator, DTO, Repository는 있었지만 Controller가 없어 `PlayerAnalysisClipViewController`를 추가했다.
* 관리용 조회 기록 API는 `GET /api/management/players/{playerId}/player-analysis-clip-views`를 사용한다.
* 선수 본인 조회 기록 API는 `GET /api/player/me/player-analysis-clip-views`를 사용한다.
* 선수 개인 분석 클립 상세 화면에 COACH, ANALYST 전용 선수 확인 기록 영역을 추가했다.
* PLAYER에게는 조회 기록 관리 UI를 노출하지 않는다.
* 조회 기록에는 조회 선수, 최초 확인 시간, 마지막 확인 시간, 대상 선수 조회 횟수를 표시한다.
* 조회 기록이 없는 경우 아직 선수가 해당 개인 분석 클립을 조회하지 않았다는 안내 문구를 표시한다.
* 조회 기록 API 실패 시 상세 영역에 사용자 친화적 에러 메시지를 표시한다.
* 조회 기록 삭제 기능은 구현하지 않았다.
* 지도자/분석관이 관리용 상세 조회를 해도 조회 기록은 증가하지 않는다.
* 조회 기록은 해당 클립의 대상 선수가 본인 개인 분석 클립 상세를 조회할 때만 생성 또는 갱신된다.
* 오해 방지를 위해 프론트 문구는 단순 `조회 횟수`가 아니라 `대상 선수 조회 횟수`로 표시한다.
...
* 선수 기록 관리 백엔드 기능을 프론트에 연결한다.
* 지도자/분석관이 선수별 기록을 관리할 수 있게 한다.
* 선수는 본인 기록만 조회할 수 있게 한다.
* PC에서는 지도자/분석관 관리 화면 사용성을 우선한다.
* 모바일에서는 선수 본인 기록 조회 사용성을 우선한다.
* 백엔드 DTO 필드와 API 주소를 실제 코드 기준으로 확인한 뒤 프론트 타입/API를 작성한다.
* 권한 검증은 반드시 백엔드에서 처리한다.
* 프론트에서는 사용자 경험을 위해 버튼과 영역만 숨긴다.
...
선수 기록 API 파일이 이미 있다면 해당 파일
선수 기록 타입 파일이 이미 있다면 해당 파일
...
backend/src/main/java/com/soccer/platform/dto/playerrecord/CreatePlayerRecordRequestDTO.java
backend/src/main/java/com/soccer/platform/dto/playerrecord/CreatePlayerRecordResponseDTO.java
backend/src/main/java/com/soccer/platform/dto/playerrecord/PlayerRecordDetailResponseDTO.java
backend/src/main/java/com/soccer/platform/dto/playerrecord/PlayerRecordListResponseDTO.java
backend/src/main/java/com/soccer/platform/dto/playerrecord/PlayerRecordPageResponseDTO.java
```
