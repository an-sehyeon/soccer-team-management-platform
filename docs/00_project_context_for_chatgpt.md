...
다음 작업으로 선수 개인 분석 클립 조회 기록 프론트 연동을 진행하는 것이 적절하다.
...
선수 개인 분석 클립 화면 구조 정렬

완료된 핵심 정책은 다음과 같다.

- 선수 개인 분석 클립 목록/상세 페이지와 등록/수정 편집기 페이지를 분리했다.
- /player-analysis-clips는 목록/상세/삭제/등록·수정 이동 전용 페이지로 정리했다.
- /player-analysis-clips/new는 등록 전용 편집기 페이지로 추가했다.
- /player-analysis-clips/:playerClipId/edit는 수정 전용 편집기 페이지로 추가했다.
...
- 선수 개인 분석 클립 + 드로잉 통합 수정 API를 추가했다.
- 통합 수정 시 제목/코멘트/드로잉만 변경하면 기존 mp4 파일을 유지한다.
- 통합 수정 시 클립 시작/종료 시간이 변경되면 mp4 파일을 재생성한다.
...
playerClipUrl은 DB의 player_video_clip.url에서 내려온다.

player_video_clip.start_time_sec, player_video_clip.end_time_sec는 DB와 Entity에 유지한다.

이 값은 원본 경기 영상에서 어떤 구간을 잘라냈는지 기록하는 생성 이력, 재생성, 디버깅용 메타데이터다.

드로잉은 영상 파일에 합성하지 않고 player_video_clip_drawing에 JSON으로 저장한다.
...
- 선수 개인 분석 클립 조회 기록 백엔드 기능을 프론트에 연결한다.
- 지도자/분석관이 선수 개인 분석 클립별 조회 기록을 확인할 수 있게 한다.
- 조회 기록에는 조회 선수, 최초 조회 시간, 마지막 조회 시간, 조회 횟수를 표시한다.
- 선수 화면에는 조회 기록 관리 UI를 노출하지 않는다.
- 조회 기록 생성은 선수가 본인 개인 분석 클립 상세를 조회할 때 백엔드에서 처리한다.
- 지도자/분석관의 관리용 상세 조회는 조회 기록에 반영하지 않는다.
- API 주소와 DTO 필드명은 실제 조회 기록 Controller/DTO를 확인한 뒤 맞춘다.
...
PlayerAnalysisClipViewService.java
PlayerAnalysisClipViewController.java 또는 조회 기록 관련 Controller
PlayerClipViewPageResponseDTO.java
PlayerClipViewResponseDTO.java
PlayerVideoClipViewEntity.java
PlayerVideoClipViewRepository.java
