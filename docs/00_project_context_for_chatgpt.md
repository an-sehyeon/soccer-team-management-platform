...
- 팀 분석 클립 등록/수정 흐름을 `/match-videos` 분석 패널로 통합
- 팀 분석 클립 기존 독립 편집 페이지 제거
- 팀 분석 클립 드로잉 canvas 위치 보정
...
- 분석 모드에서 경기 영상 상세 영역을 분석 패널이 대체하도록 정리
- 한 화면에 영상이 2개 표시되지 않도록 수정
...
```text
선수 기록 프론트 연동 및 경기 영상 기반 분석 편집 통합
```

완료된 핵심 정책은 다음과 같다.

- 선수 기록 관리 화면을 `/player-records`로 연결했다.
- 선수 기록 이벤트 조회, 수정, 삭제를 프론트에서 처리한다.
- 선수 기록 이벤트 생성은 `/match-videos?analysisMode=player-record-event`에서 처리한다.
- 팀 분석 클립 등록/수정은 `/match-videos` 분석 패널에서 처리한다.
- 선수 개인 분석 클립 등록/수정은 `/match-videos` 분석 패널에서 처리한다.
- 기존 팀/선수 분석 클립 독립 편집 페이지를 제거했다.
- 팀/선수 클립 수정 시 클립 ID로 원본 경기 영상을 자동 선택한다.
- 수정 모드에서 다른 경기 영상을 클릭하면 수정 모드를 종료하고 일반 경기 상세 모드로 전환한다.
- 분석 모드에서는 경기 영상 상세 영역을 숨기고 분석 패널만 표시한다.
- 드로잉 canvas는 영상 표시 영역과 `ResizeObserver` 기반으로 동기화한다.

완료된 주요 프론트 파일은 다음과 같다.
...
등록/수정은 `/match-videos` 분석 패널에서 처리한다.
...
프론트 canvas 좌표는 0~1 정규화 좌표를 사용한다.
...
## 20. 반복 발생한 프론트 오류와 예방 규칙

### FormEvent import 오류

`FormEvent`는 타입 전용 import로 작성한다.

```ts
import type { FormEvent } from "react";
```

### useEffect 내부 동기 setState 경고

폼 초기화용 상태 변경을 effect 시작과 동시에 반복 호출하지 않는다.

API 응답 이후에만 상태를 변경한다.

선택 데이터 변경으로 폼 초기화가 필요하면 부모 컴포넌트의 `key` 변경을 통해 remount한다.

### 분석 모드에서 영상 2개 표시 문제

분석 모드에서는 경기 영상 상세 영역을 숨기고 분석 패널이 해당 영역을 대체한다.

### 수정 모드에서 다른 경기 클릭 시 원본 불일치 문제

수정 모드에서 다른 경기 영상을 클릭하면 기존 `analysisMode` 쿼리를 제거하고 `/match-videos` 일반 상세 모드로 전환한다.

### 드로잉 위치 불일치 문제

canvas는 영상 표시 영역과 같은 부모 안에서 absolute로 겹친다.

`ResizeObserver`로 canvas 내부 크기와 표시 크기를 동기화한다.
...
- 경기 영상 원본에서 특정 시간을 북마크로 저장한다.
- 지도자/분석관이 경기 장면을 빠르게 다시 찾을 수 있게 한다.
- 북마크 제목과 메모를 저장한다.
- 북마크 시간은 원본 경기 영상 기준 초로 저장한다.
- 북마크는 경기 영상 상세 또는 분석 패널에서 사용할 수 있게 한다.
- 선수 기록 이벤트 또는 분석 클립 생성 시 북마크 시간을 활용할 수 있게 확장 가능하게 설계한다.

다음 작업에서 우선 확인해야 할 파일은 다음과 같다.

```text
docs/요구사항 문서 또는 새로 작성할 북마크 요구사항 문서
soccer_platform.sql
...
backend/src/main/java/com/soccer/platform/dto/*
```
...
- 분석 모드에서는 한 화면에 영상이 2개 표시되지 않도록 구성
