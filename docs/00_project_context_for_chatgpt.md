...
중단 이유는 선수 개인 분석 클립 저장 구조를 실제 mp4 파일 생성 방식으로 다시 설계해야 했기 때문이다.

이제 백엔드의 선수 개인 분석 클립 비동기 파일 생성 구조는 구현 및 테스트 완료된 상태다.

따라서 다음 작업에서는 중단했던 선수 개인 분석 클립 드로잉 프론트 연동을 재개한다.
...
- 기존 개별 생성 API도 `PROCESSING → READY/FAILED` 파일 생성 흐름으로 변경했다.
- 신규 통합 생성 API `POST /api/management/player-analysis-clips/with-drawings`를 추가했다.
- 생성 요청 시 `player_video_clip.status = PROCESSING`으로 먼저 저장한다.
- 트랜잭션 커밋 후 FFmpeg 비동기 작업을 실행한다.
- FFmpeg 생성 성공 시 `player_video_clip.url`에 생성된 클립 파일 URL을 저장하고 `READY`로 변경한다.
- FFmpeg 생성 실패 시 `FAILED`로 변경한다.
- 수정 API 호출 시 새 mp4 파일을 재생성한다.
- 수정 성공 후 기존 생성 파일을 삭제한다.
- 기존 파일은 수정 요청 시점에 삭제하지 않고, 새 파일 생성 성공 후 삭제한다.
- 드로잉은 영상 파일에 합성하지 않고 `player_video_clip_drawing`에 JSON으로 저장한다.
- 드로잉 시간 기준은 원본 경기 영상 기준이 아니라 생성된 클립 영상 기준 초로 변경했다.
- 원본 경기 영상이 소프트 삭제되어도 이미 생성된 선수 개인 분석 클립은 독립 재생 가능하다.
...
드로잉의 `startTimeSec`, `endTimeSec`는 생성된 클립 영상 기준이다.
...
선수 개인 분석 클립 상세 응답에는 더 이상 아래 필드를 내려주지 않는다.
...
```text
선수 개인 분석 클립 드로잉 프론트 연동 재개
```

다음 작업의 핵심 방향은 다음과 같다.

- 백엔드의 선수 개인 분석 클립 비동기 파일 생성 구조에 맞춰 프론트를 수정한다.
- 선수 개인 분석 클립 상세 재생 기준을 원본 경기 영상 URL이 아니라 `playerClipUrl`로 변경한다.
- `PROCESSING`, `READY`, `FAILED` 상태를 목록/상세 화면에 표시한다.
- `READY` 상태에서만 영상 재생과 드로잉 오버레이를 활성화한다.
- `PROCESSING`, `FAILED` 상태에서는 재생과 드로잉 표시를 막는다.
- 드로잉 시간은 생성된 클립 영상 기준 초로 처리한다.
- 통합 생성 API `POST /api/management/player-analysis-clips/with-drawings`를 사용한다.
- 기존 원본 경기 영상 URL + `startTimeSec`, `endTimeSec` 구간 재생 로직은 제거하거나 신규 구조에 맞게 정리한다.
- 프론트 타입은 변경된 백엔드 DTO 기준으로 다시 맞춘다.
