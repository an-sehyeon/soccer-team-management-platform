...
5. `playerId`가 존재하는지 확인한다.
6. 대상 회원이 삭제되지 않은 `PLAYER`인지 확인한다.
7. 요청 필수값을 검증한다.
8. `clipType`이 허용된 Enum인지 확인한다.
9. `startTimeSec`와 `endTimeSec`가 0 이상인지 확인한다.
10. `startTimeSec < endTimeSec`인지 확인한다.
11. 실제 영상을 자르지 않으므로 `url`은 null로 저장한다.
12. 메타데이터 기반 클립이므로 `status`는 `READY`로 저장한다.
13. `editor_id`에 등록한 회원 ID를 저장한다.
14. `player_id`에 대상 선수 회원 ID를 저장한다.
15. 선수 개인 분석 클립을 저장한다.
16. 생성된 선수 개인 분석 클립 ID를 반환한다.
...
6. `playerId`가 존재하고 삭제되지 않은 `PLAYER`인지 검증한다.
7. 요청 필수값을 검증한다.
8. `clipType`이 허용된 Enum인지 확인한다.
9. `startTimeSec < endTimeSec`인지 확인한다.
10. 선수 개인 분석 클립 정보를 수정한다.
11. 수정된 선수 개인 분석 클립 정보를 반환한다.
...
6. 선수 개인 분석 클립 수정 기능 작성
7. 선수 개인 분석 클립 삭제 기능 작성
...
6. 존재하지 않는 `playerId`로 등록 실패 테스트
7. `playerId`가 `COACH`이면 등록 실패 테스트
8. `playerId`가 `ANALYST`이면 등록 실패 테스트
9. `startTimeSec >= endTimeSec` 등록 실패 테스트
10. 지도자 관리용 목록 조회 성공 테스트
11. 분석관 관리용 목록 조회 성공 테스트
12. 선수 관리용 목록 조회 실패 테스트
13. 선수 본인 개인 분석 클립 목록 조회 성공 테스트
14. 선수 본인 개인 분석 클립 상세 조회 성공 테스트
15. 선수의 다른 선수 개인 분석 클립 상세 조회 실패 테스트
16. 지도자 선수 개인 분석 클립 수정 성공 테스트
17. 분석관 선수 개인 분석 클립 수정 성공 테스트
18. 선수 선수 개인 분석 클립 수정 실패 테스트
19. 지도자 선수 개인 분석 클립 삭제 성공 테스트
20. 분석관 선수 개인 분석 클립 삭제 실패 테스트
21. 선수 선수 개인 분석 클립 삭제 실패 테스트
22. 삭제된 선수 개인 분석 클립 조회 제외 테스트
23. 페이징 조회 테스트
24. 클립 유형 필터 조회 테스트
25. 원본 경기 영상별 필터 조회 테스트
26. 대상 선수별 필터 조회 테스트
