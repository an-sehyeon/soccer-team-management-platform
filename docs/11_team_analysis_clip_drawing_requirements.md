...
팀 분석 클립 드로잉 기능은 지도자와 분석관이 팀 분석 클립 위에 전술 설명용 드로잉 데이터를 저장하고, 선수는 해당 드로잉을 조회할 수 있게 하는 기능이다.

초기 구현은 다음 정책을 기준으로 진행한다.

* 인증은 기존 JWT 인증 구조를 사용한다.
* JWT 인증 후 `CustomUserPrincipal`에서 `memberId`, `memberRole`, `isAdmin` 값을 사용한다.
* 드로잉 데이터는 `team_video_clip_drawing` 테이블에 저장한다.
* 드로잉은 프론트 캔버스에서 생성한 좌표, 텍스트, 도형 데이터를 JSON으로 저장한다.
* 지도자 `COACH`는 드로잉 등록, 조회, 수정, 삭제가 가능하다.
* 분석관 `ANALYST`는 드로잉 등록, 조회, 수정이 가능하고 삭제는 불가능하다.
* 선수 `PLAYER`는 드로잉 조회만 가능하다.
* 삭제는 실제 삭제가 아니라 `is_deleted = true`로 처리한다.
* 드로잉 생성 시 `teamClipId`가 존재하고 삭제되지 않았는지 반드시 백엔드에서 검증한다.
* 드로잉의 `startTimeSec`, `endTimeSec`는 연결된 팀 분석 클립의 `startTimeSec`, `endTimeSec` 범위 안에 있어야 한다.
...
팀 분석 클립 드로잉 기능의 목적은 팀 분석 클립의 특정 시간대에 전술적 의미를 시각적으로 표시하는 것이다.

예를 들어 다음과 같은 분석을 표현할 수 있다.
...
이 기능은 단순 영상 조회가 아니라, 지도자와 분석관이 선수에게 “어디를 봐야 하는지”를 명확히 전달하기 위한 핵심 분석 기능이다.
...
분석관에게 삭제 권한을 주지 않는 이유는 드로잉 데이터가 선수에게 공유되는 분석 자료이고, 실수로 삭제될 경우 클립의 분석 의도가 사라질 수 있기 때문이다.
...
* `COACH`, `ANALYST`, `PLAYER` 모두 드로잉을 조회할 수 있다.
...
1. 지도자 또는 분석관이 팀 분석 클립 상세 화면에 진입한다.
2. 특정 시간대에서 영상을 멈춘다.
3. 캔버스 위에 선, 화살표, 원, 박스, 영역, 텍스트를 추가한다.
4. 프론트는 드로잉 데이터를 JSON으로 만든다.
5. 서버에 `teamClipId`, `drawingType`, `startTimeSec`, `endTimeSec`, `drawingData`를 전송한다.
6. 서버는 팀 분석 클립 존재 여부와 시간 범위를 검증한다.
7. 검증 통과 시 드로잉을 저장한다.

### 5.2 선수 드로잉 조회 흐름

1. 선수가 팀 분석 클립 상세 화면에 진입한다.
2. 클립의 드로잉 목록을 조회한다.
3. 영상 재생 시간이 드로잉의 `startTimeSec`, `endTimeSec` 범위 안에 들어오면 프론트에서 해당 드로잉을 캔버스에 표시한다.
...
3. `teamClipId`로 팀 분석 클립을 조회한다.
4. 팀 분석 클립이 존재하지 않거나 삭제된 경우 실패 처리한다.
5. `is_deleted = false`인 드로잉만 조회한다.
6. `start_time_sec ASC`, `id ASC` 기준으로 정렬한다.
7. 드로잉 목록을 반환한다.
...
4. 연결된 팀 분석 클립이 존재하지 않거나 삭제된 경우 실패 처리한다.
5. 드로잉 상세 정보를 반환한다.
...
8. 드로잉 시간이 팀 분석 클립 시간 범위 안에 있는지 확인한다.
9. `drawingData`가 비어 있지 않은지 확인한다.
10. `member_id`에 등록한 회원 ID를 저장한다.
11. 드로잉을 저장한다.
12. 생성된 드로잉 ID를 반환한다.
...
5. 연결된 팀 분석 클립이 존재하지 않거나 삭제된 경우 실패 처리한다.
6. 요청 값을 검증한다.
7. 드로잉 시간이 팀 분석 클립 시간 범위 안에 있는지 확인한다.
8. 드로잉 정보를 수정한다.
9. 수정된 드로잉 정보를 반환한다.
...
초기 구현에서는 현재 `team_video_clip_drawing` 테이블을 그대로 사용한다.

| 컬럼                   | 사용 목적                      |
| -------------------- | -------------------------- |
| `id`                 | 드로잉 PK                     |
| `team_video_clip_id` | 팀 분석 클립 ID                 |
...
| -------------------- | --------------- |
| `id`                 | `id`            |
| `team_video_clip_id` | `teamVideoClip` |
| `member_id`          | `member`        |
...
`CreateTeamAnalysisClipDrawingRequestDTO`
...
`UpdateTeamAnalysisClipDrawingRequestDTO`
...
`TeamAnalysisClipDrawingResponseDTO`
...
`CreateTeamAnalysisClipDrawingResponseDTO`
...
| 존재하지 않는 팀 분석 클립 ID   | 등록/조회 실패    | `404 Not Found`    |
| 삭제된 팀 분석 클립 ID       | 등록/조회 실패    | `404 Not Found`    |
...
| 드로잉 시간이 클립 범위를 벗어남   | 요청 실패       | `400 Bad Request`  |
...
1. `TeamVideoClipDrawingEntity` 필드와 DB 컬럼 매핑 확인
2. `TeamVideoClipEntity`와 `TeamVideoClipDrawingEntity` 연관관계 확인
3. `MemberEntity`와 `TeamVideoClipDrawingEntity` 연관관계 확인
4. 기존 `TeamVideoClipRepository` 확인
5. 기존 공통 예외 구조와 `ErrorCode` 확인

### 11.2 백엔드 2단계: Enum 작성

1. `DrawingTypeEnum` 생성 또는 확인
2. 허용 값은 `LINE`, `ARROW`, `CIRCLE`, `BOX`, `AREA`, `TEXT`로 제한

### 11.3 백엔드 3단계: DTO 작성

1. `CreateTeamAnalysisClipDrawingRequestDTO` 작성
2. `UpdateTeamAnalysisClipDrawingRequestDTO` 작성
3. `CreateTeamAnalysisClipDrawingResponseDTO` 작성
4. `TeamAnalysisClipDrawingResponseDTO` 작성

### 11.4 백엔드 4단계: Repository 작성

1. `TeamVideoClipDrawingRepository` 작성
2. `findByTeamVideoClipAndIsDeletedFalseOrderByStartTimeSecAscIdAsc()` 계열 메서드 작성
...
6. `isAdmin`만으로 드로잉 변경 권한을 주지 않도록 처리

### 11.6 백엔드 6단계: 팀 분석 클립 검증 로직 작성

1. `teamClipId`로 `team_video_clip` 조회
2. 존재하지 않으면 실패 처리
3. `isDeleted = true`이면 실패 처리
4. 드로잉 등록/조회 시 반드시 이 검증 메서드 사용
...
4. `drawing.startTimeSec >= teamClip.startTimeSec` 검증
5. `drawing.endTimeSec <= teamClip.endTimeSec` 검증
...
2. 분석관 드로잉 등록 성공 테스트
3. 선수 드로잉 등록 실패 테스트
4. 존재하지 않는 `teamClipId`로 등록 실패 테스트
5. 삭제된 `teamClipId`로 등록 실패 테스트
6. `startTimeSec >= endTimeSec` 등록 실패 테스트
7. 드로잉 시간이 클립 범위를 벗어나는 경우 실패 테스트
8. 전체 역할 드로잉 목록 조회 성공 테스트
9. 전체 역할 드로잉 상세 조회 성공 테스트
10. 지도자 드로잉 수정 성공 테스트
11. 분석관 드로잉 수정 성공 테스트
12. 선수 드로잉 수정 실패 테스트
13. 지도자 드로잉 삭제 성공 테스트
14. 분석관 드로잉 삭제 실패 테스트
15. 선수 드로잉 삭제 실패 테스트
16. 삭제된 드로잉 조회 제외 테스트

---

## 12. 추후 확장 가능성

초기 구현에서는 JSON 저장 기반 드로잉 CRUD에 집중한다.
...
* 드로잉 생성 시 `teamClipId`가 존재하고 삭제되지 않았는지 반드시 검증한다.
* 드로잉 시간은 연결된 팀 분석 클립 시간 범위를 벗어나면 안 된다.
