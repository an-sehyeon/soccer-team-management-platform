...
> [!IMPORTANT]
> 경기 영상 편집기 v1 이후 영상·클립 삭제와 시간 정밀도 정책은
...
경기 영상 편집기 v1의 신규 canonical 시간은 밀리초다.

- 팀 분석 클립 시작·종료 시간: 밀리초
- 선수 개인 분석 클립 시작·종료 시간: 밀리초
- 선수 기록 이벤트의 클립 구간 스냅샷: 밀리초
- 기존 초 단위 데이터: `기존 값 × 1000`으로 마이그레이션

정확한 신규 컬럼명과 DTO 전환 순서는 실제 Entity와 SQL 검토 후 확정한다.

프론트 전환 전 기존 초 단위 응답이 필요하면 백엔드가 밀리초 값을 기준으로 호환 필드를 파생한다. 신규 로직의 원본 데이터는 밀리초 값을 사용한다.

### 0.2 직접 클립 영구 삭제

COACH 또는 ANALYST가 팀·선수 분석 클립을 직접 영구 삭제하면 다음 연결을 삭제한다.
...
연결 행이 삭제되므로 해당 이벤트 화면에서는 삭제된 클립으로 이동하는 버튼이나 링크를 제공하지 않는다.

### 0.3 원본 경기 영상 삭제로 인한 팀 클립 삭제

원본 경기 영상 삭제 시 연결된 팀 분석 클립은 소프트 삭제되고 결과 MP4는 즉시 삭제 대상이 된다.

이 경우에는 선수 기록 이벤트와 클립 연결 이력을 유지한다.
...
선수 기록 이벤트에는 READY 상태의 팀 또는 선수 개인 분석 클립만 새로 연결할 수 있다.
...
상태의 클립은 연결 후보 목록과 등록 API에서 제외한다.

### 0.5 정책 우선순위

- 이벤트 생성·중복·요약 수치 반영 정책은 기존 본문을 유지한다.
- 영상 상태, 삭제, 시간 단위와 접근 정책이 충돌하면 이 0번 정책과 31번 요구사항을 우선한다.
