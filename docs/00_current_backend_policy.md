...
선수 기록 요약, 선수 기록 이벤트, 선수 기록 이벤트와 클립 연결도 관리할 수 있다.
...
단, 선수 기록 기능에서는 기존 최종 정책상 분석관도 기록 삭제가 가능하다.

따라서 선수 기록 요약, 선수 기록 이벤트, 선수 기록 이벤트와 클립 연결에 대해서는 분석관도 등록, 조회, 수정, 삭제할 수 있다.
...
선수는 본인 선수 기록 요약, 본인 선수 기록 이벤트, 본인 기록 이벤트에 연결된 클립만 조회할 수 있다.
...
팀 분석 클립은 실제 mp4 파일 생성 구조를 사용한다.
...
팀 분석 클립은 선수 기록 이벤트와 연결할 수 있다.

팀 분석 클립은 팀 전체 클립이므로 기록 대상 선수는 선수 기록 이벤트 등록 시 별도로 선택한다.
...
선수 개인 분석 클립은 선수 기록 이벤트와 연결할 수 있다.

단, 선수 개인 분석 클립을 기록 이벤트와 연결할 때는 클립 대상 선수와 기록 대상 선수가 일치해야 한다.

---

## 11. 선수 개인 분석 클립 조회 기록 정책
...
`player_record`는 같은 `upload_id`, `player_id` 기준으로 활성 row를 1개만 유지한다.

기록 이벤트 등록 시 해당 경기와 선수의 `player_record`가 없으면 자동 생성한다.

이미 존재하면 새 `player_record`를 만들지 않고 기존 `player_record`의 요약 수치를 갱신한다.

`player_record.memo`는 경기 전체 또는 해당 선수의 경기 전체 기록에 대한 메모다.

---

## 13. 선수 기록 이벤트 정책

신규 `player_record_event`는 특정 기록 이벤트를 저장한다.

`player_record_event`는 기록이 등록될 때마다 새로 저장한다.

`player_record_event`에는 `upload_id`, `player_id`를 저장하지 않는다.

이 값은 `player_record_id`를 통해 `player_record`에서 확인한다.

`player_record_event.memo`는 이벤트 단위 메모다.
...
## 14. 선수 기록 이벤트와 클립 연결 정책

신규 `player_record_event_clip`은 기록 이벤트와 클립을 연결한다.

`player_record_event_clip`은 필수가 아니라 선택 연결 테이블이다.

따라서 기록 이벤트는 클립 없이도 존재할 수 있다.
...
팀 분석 클립과 선수 개인 분석 클립 모두 선수 기록 이벤트와 연결할 수 있다.

선수 개인 분석 클립 연결 시 기록 대상 선수와 클립 대상 선수가 일치해야 한다.

클립 없는 기록 이벤트 등록 API와 클립 연결 포함 기록 이벤트 등록 API를 분리한다.
...
프론트에서는 READY 상태 클립만 정상 재생 기준으로 처리한다.
...
선수 기록 이벤트 작업에서 추가 검토할 예외는 다음과 같다.
...
```text
soccer_platform.sql
PlayerRecordEntity.java
PlayerRecordRepository.java
PlayerRecordService.java
PlayerRecordValidator.java
PlayerRecordController.java
...
GameVideoUploadRepository.java
MemberEntity.java
