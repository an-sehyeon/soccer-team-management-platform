...
// 선수 개인 분석 클립 파일 생성 서비스
// FFmpeg를 실행해 원본 경기 영상에서 특정 구간을 mp4 파일로 생성

@Service
public class PlayerAnalysisClipFileGenerator {
	 ...
	 * 4. 생성 파일 존재 여부 검증
	 */
	public void generatePlayerAnalysisClipFile(PlayerAnalysisClipGenerationCommand command) {
	...
	// FFmpeg 명령어 생성
	private List<String> createFfmpegCommand(PlayerAnalysisClipGenerationCommand command) {
		return List.of(
				ffmpegPath,
				"-y",
				"-ss",
	...
	// 생성 명령 값 검증
	private void validateGenerationCommand(PlayerAnalysisClipGenerationCommand command) {
		if (command == null
				|| command.getPlayerClipId() == null
	...
	// 생성된 파일 검증
	private void validateGeneratedFile(PlayerAnalysisClipGenerationCommand command) {
		try {
